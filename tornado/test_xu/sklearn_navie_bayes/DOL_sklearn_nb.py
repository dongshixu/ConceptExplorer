"""
@Author: Dustin Xu
@Date: 2020/3/6 20:26 AM
@Description:  使用sklearn-naive-bayes
"""
...
class DistributedOnlineLearning:
    def __init__(self, con, data_length=2):
        self.con = con
        self.attributes = []
        self.le = data_length
    ...
    def sub_thread(self, dataset, sub_dataset, spilt_inform, folder_create, length):
        ...
        self.nb_set[sub_dataset] = MultinomialNB()
        self.last_wl_status[sub_dataset] = dict(r_l=0, hit=[])
        self.instance_set[sub_dataset] = dict(train=[[0, 1, 2], [0, 1, 2]], label=[0, 1])
                        ...
                        if global_count == length:
                            ...
                            if len(self.data_statistics[sub_dataset]['delay']['warningLevel']) > 2:
                                for data_set_name in d_s_n:
                                    ...
                                    self.instance_set[data_set_name]['train'].append([self.wl_transformer(self.data_statistics[value]['delay']['warningLevel'][-2]['max'])
                                                                                      for value in d_s_n if value != data_set_name])
                                    self.instance_set[data_set_name]['label'].append(max(self.last_wl_status[data_set_name]['hit']))
                                ...
                                if len(self.data_statistics[sub_dataset]['delay']['warningLevel']) > 3:
                                    ...
                                    for temple_name in d_s_n:
                                        ...
                                        self.nb_set[temple_name].fit(self.instance_set[temple_name]['train'][:-1], self.instance_set[temple_name]['label'][:-1])
                                        predict = self.nb_set[temple_name].predict([self.instance_set[temple_name]['train'][-1]])[0]
                                        prob = self.nb_set[temple_name].predict_proba([self.instance_set[temple_name]['train'][-1]])[0][-1]

                                        self.data_statistics[temple_name]['delay']['nb_prob'].append(prob)
                                        self.nb_drift_prob[temple_name]['prob'].append(prob)
                                        self.nb_drift_prob[temple_name]['ground_truth'].append(
                                            self.data_statistics[temple_name]['delay']['warningLevel'][-2]['max'])

                                        if predict == self.instance_set[temple_name]['label'][-1]:
                ...
                if __instance_count == __data_length:  # 最后一个batch可能只有少部分数据，要考虑
                    ...
                    if global_count == length:
                        ...
                        for data_set_name in d_s_n:
                            ...
                            #                                     for value in d_s_n if value != data_set_name] \
                            #                                    + [max(self.last_wl_status[data_set_name]['hit'])]
                            self.instance_set[data_set_name]['train'].append(
                                [self.wl_transformer(self.data_statistics[value]['delay']['warningLevel'][-2]['max'])
                                 for value in d_s_n if value != data_set_name])
                            self.instance_set[data_set_name]['label'].append(
                                max(self.last_wl_status[data_set_name]['hit']))
                        # testing
                        for temple_name in d_s_n:
                            self.nb_set[temple_name].fit(self.instance_set[temple_name]['train'][:-1],
                                                         self.instance_set[temple_name]['label'][:-1])
                            predict = self.nb_set[temple_name].predict([self.instance_set[temple_name]['train'][-1]])[0]
                            prob = \
                            self.nb_set[temple_name].predict_proba([self.instance_set[temple_name]['train'][-1]])[0][-1]
...
if __name__ == "__main__":
    ...
    for ds in Dataset.DATASET:
        ...
        dol = DistributedOnlineLearning(lock_con, data_length=len(sub_data_set_list)-1)
