"""
@Author: Dustin Xu
@Date: 2020/2/22 21:25 AM
@Description: [t - delta_t, t + delta_t] naive bayes
"""
...
def calculate_warning_level(warning_list, warning_output, hit_list, repeated_flag=False):
    if repeated_flag:
...
def calculate_correlation(matrix, last_batch, attr_dict, SI, label_correlation, batch_correlation, repeat_flag=False):
    if repeat_flag:
        ...
        count = 0
        if last_batch is None:
            last_batch_nor = normalization_attribution(matrix, SI)
        else:
            _matrix = normalization_attribution(matrix, SI)
            for j in range(len(last_batch)):
                batch_c = round(cosin_relation(last_batch[j], _matrix[j]), 4)
                batch2batch.append(batch_c)
            batch_correlation = batch2batch
            last_batch_nor = _matrix
        for i in range(np.shape(matrix)[1]-1):
            c = round(cosin_relation(matrix[:, i], label), 4)
...
if __name__ == '__main__':
    for dataset in Dataset.DATASET:
        ...
        for sub_dataset in Dataset.get_sub_dataset(dataset):
                ...
                if __instance_count > __window_size and date_time_flag and prsa_flag:
                    if dataset == 'prsa_data':
                        ...
                        if __instance_count == __window_size + 1:
                            __start_point = unix_time
                            batch_start_time.append(__start_point)
                            # print(instance[-2], batch_start_time)
                    ...
                    if difference_value >= __batch_size:
                        ...
                        for cc in range(batch_interval):
                            ...
                            # 属性记录
                            attributes_set, last_batch_attributions, lc, bc = calculate_correlation(
                            ...
                            # warning level
                            warning_level, warning_level_set, hit = calculate_warning_level(
                                warning_level_set, warning_level, hit, repeated_flag=r_f)
                            ...
                            if len(warning_level) > 1:
                                example = [wl_transformer(warning_level[-2]['max']), hit[-1]]
                                if len(warning_level) > 2:
                                    nb_learner.set_ready()
                                    predicted = nb_learner.do_testing(example)
                                    # print(predicted, example[-1])
                                    if predicted == example[-1]:
                                        __nb_count += 1
                                    __nb_all_count += 1
                                    accuracy_nb[sub_dataset].append(round((__nb_count / __nb_all_count), 4))
                                    nb_learner.do_training(example, drift_status)
                                else:
                                    nb_learner.do_training(example, drift_status)
                    ...
                    # if prediction_status:  # 还没想好怎么添加bingo的值
                    #     __bingo += 1
                    ...
                    if __instance_count == __data_length:  # 最后一个batch可能只有少部分数据，要考虑
                        ...
                        # warning level
                        warning_level, warning_level_set, hit = calculate_warning_level(
                            warning_level_set, warning_level, hit, repeated_flag=False)
                        example = [wl_transformer(warning_level[-2]['max']), hit[-1]]
                        nb_learner.set_ready()
                        predicted = nb_learner.do_testing(example)
                        # print(predicted, example[-1])
                        ...
                        accuracy_nb[sub_dataset].append(round((__nb_count / __nb_all_count), 4))
                        nb_learner.do_training(example, drift_status)
                    # if len(batch_start_time) == 46:
                    #     print("1-Pmin, accuracy", drift_detector.o_s_d_min, drift_detector.accuracy)
            ...
            print(len(batch_start_time), len(accuracy), len(warning_level), len(hit))

            save_path = sub_folder_path + '/'
            save_file(configure, save_path, type_name='configure')
            save_file(data_statistics, save_path, type_name=sub_dataset)
            evaluator.plot(step=__step, dataset=dataset, data=sub_dataset)
            evaluator.store_stats()

        Zip(folder_create.get_path())
        nb_accuracy_path = folder_create.get_path() + '/'
        save_file(accuracy_nb, folder_create.get_path(), type_name='naive_bayes_accuracy')
        Zip.plot(accuracy_nb)
