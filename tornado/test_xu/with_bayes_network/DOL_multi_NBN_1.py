"""
@Author: Dustin Xu
@Date: 2020/3/5 11:24 AM
@Description:  使用贝叶斯网络(用其它数据源是否发生drift预测当前数据源)
"""
...
class DistributedOnlineLearning:
    ...
    def construct_attribute(self):
        for attr in [('warning_level', 3, [0, 1, 2])]:
    ...
    def sub_thread(self, dataset, sub_dataset, spilt_inform, folder_create, sub_data_set):
        ...
        dag = []
        for name in sub_data_set:
            if name != sub_dataset:
                dag.append((name, sub_dataset))
        self.nb_set[sub_dataset] = BayesianModel(dag)
        self.last_wl_status[sub_dataset] = dict(r_l=0, hit=[])

        self.real_status[sub_dataset] = [0, 1]
        self.risk_level[sub_dataset] = [0, 1, 2, 0, 1, 2]
        ...
        __detect_interval = Dataset.get_detect_batch_interval(dataset, sub_dataset)
            ...
            if __instance_count > __window_size and date_time_flag and prsa_flag:
                        ...
                        if global_count == length:
                            ...
                            if len(self.data_statistics[sub_dataset]['delay']['warningLevel']) >= 2:
                                for data_set_name in d_s_n:
                                    self.real_status[data_set_name].append(max(self.last_wl_status[data_set_name]['hit'][-2:]))
                                    self.risk_level[data_set_name].append(self.wl_transformer(self.data_statistics[data_set_name]['delay']['warningLevel'][-2]['max']))
                                # 构建数据预测：
                                for sub_name in d_s_n:
                                    pd_data = []
                                    for name in d_s_n:
                                        if sub_name == name:
                                            pd_data.append(self.real_status[name])
                                        else:
                                            pd_data.append(self.risk_level[name])
                                    pd_data = pd.DataFrame(np.array(pd_data).T, columns=list(d_s_n))
                                    train_data = pd_data[:-1].copy()
                                    test_data = pd_data[-1:].copy()

                                    self.nb_set[sub_name].fit(train_data)
                                    test_data.drop(sub_name, axis=1, inplace=True)
                                    prob = self.nb_set[sub_name].predict_probability(test_data).iat[0, 1]
                                    if prob > 0.5:
                                        predict = 1
                                    else:
                                        predict = 0

                                    # print(sub_name, predict, self.real_status[sub_name][-1])

                                    self.data_statistics[sub_name]['delay']['nb_prob'].append(prob)
                                    self.nb_drift_prob[sub_name]['ground_truth'].append(self.plot_risk_level[sub_name])
                                    self.nb_drift_prob[sub_name]['prob'].append(prob)

                                    if predict == self.real_status[sub_name][-1]:
                                        self.nb_classifier_accuracy[sub_name]['right_count'] += 1
                                    self.nb_classifier_accuracy[sub_name]['all_count'] += 1
                                    self.nb_classifier_accuracy[sub_name]['accuracy'].append(
                                        round(self.nb_classifier_accuracy[sub_name]['right_count'] / self.nb_classifier_accuracy[sub_name]['all_count'], 4))
                ...
                if __instance_count == __data_length:  # 最后一个batch可能只有少部分数据，要考虑
                    ...
                    if global_count == length:
                        ...
                        for data_set_name in d_s_n:
                            self.real_status[data_set_name].append(max(self.last_wl_status[data_set_name]['hit'][-2:]))
                            self.risk_level[data_set_name].append(self.wl_transformer(
                                self.data_statistics[data_set_name]['delay']['warningLevel'][-2]['max']))
                        # 构建数据预测：
                        for sub_name in d_s_n:
                            pd_data = []
                            for name in d_s_n:
                            ...
                            self.nb_classifier_accuracy[sub_name]['all_count'] += 1
                            self.nb_classifier_accuracy[sub_name]['accuracy'].append(
                                round(self.nb_classifier_accuracy[sub_name]['right_count'] /
                                      self.nb_classifier_accuracy[sub_name]['all_count'], 4))
                        ...
                        for key_name in self.data_statistics.keys():
                            ...
                            self.configure[key_name]['dataNumMax'] = max(self.data_statistics[key_name]['online']['dataNum'])
...
if __name__ == "__main__":
    ...
    for ds in Dataset.DATASET:
        ...
        for sds_id, sds in enumerate(sub_data_set_list):
            t = threading.Thread(target=dol.sub_thread, args=(ds, sds, information, f_c, sub_data_set_list))
            t.start()
            threads.append(t)
