"""
@Author: Dustin Xu
@Date: 2020/3/01 14:03 PM
@Description:  Use all data sources as bayesian training data
"""
...
class DistributedOnlineLearning:
    def __init__(self, con, data_length=3):
        self.con = con
        self.length = data_length  # if 全数据源  else: self.length = data_length - 1
        self.attributes = []
    ...
    def construct_attribute(self):
        for attr in [('warning_level', self.length, [0, 1, 2])]:
    ...
    def sub_thread(self, dataset, sub_dataset, spilt_inform, folder_create, ds_len):
                        ...
                        if global_count == ds_len:
                            ...
                            if len(self.data_statistics[sub_dataset]['delay']['warningLevel']) >= 2:
                                for data_set_name in d_s_n:
                                    self.instance_set[data_set_name] = [self.last_wl_status[value]['r_l'] for value in d_s_n] \
...
if __name__ == "__main__":
    ...
    for ds in Dataset.DATASET:
        ...
        length = len(sub_data_set_list)
        dol = DistributedOnlineLearning(lock_con, data_length=length)
        for sds_id, sds in enumerate(sub_data_set_list):
            t = threading.Thread(target=dol.sub_thread, args=(ds, sds, information, f_c, length))
            t.start()
            threads.append(t)
