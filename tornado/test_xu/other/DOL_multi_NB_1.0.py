            ...
            if __instance_count > __window_size and date_time_flag and prsa_flag:
                ...
                if difference_value >= __batch_size:
                    ...
                    for cc in range(batch_interval):
                        ...
                        self.last_wl_status[sub_dataset]['r_l'] = self.wl_transformer(warning_level[-1]['max'])
                        if len(self.last_wl_status[sub_dataset]['hit']) >= 2:
                            self.last_wl_status[sub_dataset]['hit'].pop(0)
                            self.last_wl_status[sub_dataset]['hit'].append(hit[-1])
                        else:
                            self.last_wl_status[sub_dataset]['hit'].append(hit[-1])

                        global_count += 1

                        self.plot_risk_level[sub_dataset] = warning_level[-1]['max']
                            ...
                            if len(warning_level) >= 2:
                                ...
                                if len(warning_level) >= 3:
                                    ...
                                    for temple_name in d_s_n:
                                        ...
                                        self.nb_drift_prob[temple_name]['ground_truth'].append(self.plot_risk_level[temple_name])
                ...
                if __instance_count == __data_length:  # 最后一个batch可能只有少部分数据，要考虑
                    ...
                    if global_count == length:
                        ...
                        save_path = sub_folder_path + sub_dataset
                        self.save_file(configure, save_path, type_name='configure')
                        self.save_file(data_statistics, save_path, type_name=sub_dataset)
