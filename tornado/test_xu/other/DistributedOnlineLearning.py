"""
@Author: Dustin Xu
@Date: 2020/2/14 9:58 PM
@Description: distributed online learning with Naive Bayes
"""
...
class DistributedOnlineLearning:
    def __init__(self, leaner=None, drift_detector=None, evaluator=None):
    ...
    def construct_attribute(self):
        for attr in [('warning_level', 1, [1, 2, 3]), ('data_number', 1, [0, 1, 2, 3])]:
    ...
    def run(self, data_set, sub_data_set, information, con):
        global global_count

        self.last_wl_status[sub_dataset] = 0
        ...
        self.attributes_set = self.construct_correlation(data_set)
        self.spilt_inform = information
        ...
        for x, y, attribute in data.data(batch_size=1):
            ...
            if self.__instance_count > self.__window_size and self.date_time_flag and self.prsa_flag:
                if data_set == 'prsa_data':
                    ...
                    if self.__instance_count == self.__window_size + 1:
                        self.__start_point = self.unix_time
                        self.batch_start_time.append(self.__start_point)
    ...
    def calculate_warning_level(self):
        ...
        # self.current_warning_level = _max
        self.last_wl_status[sub_dataset] = _max
        self.warning_level_set = []
